...
class LLM:
    ...
    def _generate_openai(self, messages):
        """
        使用 OpenAI chatGPT 模型生成。

        :param messages: 包含系统提示和用户内容的消息列表。
        :return: 生成的报告内容。
        """
        LOG.info(f"使用 OpenAI的 {self.config.openai_model_name} 模型生成。")
        try:
    ...
    def _generate_ollama(self, messages):
        ...
        :return: 生成的报告内容。
        """
        LOG.info(f"使用 Ollama的 {self.config.ollama_model_name} 模型生成。")
        try:
            payload = {
...
if __name__ == '__main__':
    ...
    github_report = llm.generate(system_prompt, markdown_content)
